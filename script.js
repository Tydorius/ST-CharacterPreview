(function() {
     ...
     */
    function handleStartChat(characterId) {
        log(`Starting chat with character ID: ${characterId}`);

        // Close the modal first
        closeModal();

        // Get context and select the character
        const context = SillyTavern.getContext();
        if (context && context.selectCharacterById) {
            context.selectCharacterById(String(characterId));
            log('Chat started successfully');
        } else {
            console.error('[Character Details Popup] Unable to start chat - selectCharacterById not available');
        }
    }
    ...
    function setupCharacterClickInterception() {
        ...
        characterListContainer.addEventListener('click', function(event) {
            ...
            if (characterCard) {
                ...
                // Extract character filename from data attribute
                ...
                if (characterId) {
                    ...
                    if (!context || !context.characters) {
                        console.error('[Character Details Popup] Unable to access character data from context');
                        return;
                    }
     ...
     */
    function init() {
        // Get the event source from SillyTavern
        const eventSource = window.SillyTavern?.getContext?.()?.eventSource;

        if (!eventSource) {
            console.error('[Character Details Popup] Could not access SillyTavern event source');
            return;
        }
